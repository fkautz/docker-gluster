...
RUN yum --setopt=tsflags=nodocs -y install wget ; yum clean all
RUN yum --setopt=tsflags=nodocs -y install nfs-utils ; yum clean all
...
RUN yum --setopt=tsflags=nodocs -y install glusterfs glusterfs-server glusterfs-fuse glusterfs-geo-replication ; yum clean all
